...
frappe.ui.form.on('Sales Invoice', {
	/*refresh(frm) {
		// your code here
		console.log("This is the sales invoice")
		...
		frappe.db.get_list('Sales Invoice', {
			...
			console.log(results)
		  	//frappe.msgprint(__());  
		  	frappe.msgprint(__(results));
		});

	},*/
	
	after_save: function(frm) {
        // Set the invoice number in a custom field 'custom_invoice_number'
        if (frm.doc.custom_bar_code == null || frm.doc.custom_bar_code == "") {
            frm.set_value('custom_bar_code', frm.doc.name);
        }
    },
	on_submit: function(frm) {
        // Check if the QR code field is populated
        if (frm.doc.custom_qr_code) {
            // Display the QR code image inside the custom_qr_code field
            frm.fields_dict.custom_qr_code.$wrapper.html(
                `<img src="${frm.doc.custom_qr_code}" width="150px" height="150px" alt="QR Code">`
            );
        }
    }
