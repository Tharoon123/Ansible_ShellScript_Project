...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
		...
		erpnext_base_url = "http://192.168.8.117"
		pdf_url = f"{erpnext_base_url}/api/method/frappe.utils.print_format.download_pdf?doctype=Sales%20Invoice&name={self.name}&format=test&no_letterhead=1&letterhead=No%20Letterhead&settings=%7B%7D&_lang=en-GB"

		api_key = "041855c4f1db061"  # Replace with your actual API Key
		api_secret = "d780364ad2cf41d"  # Replace with your actual API Secret
		...
		headers = {"Authorization": f"Basic {base64_string}"}  # Use Basic Authentication
		...
		pdf_filename = f"Sales_Invoice_{self.name}.pdf"

		# Now upload to S3
		s3.put_object(Bucket=s3_bucket, Key=pdf_filename, Body=pdf_content, ACL='public-read')
		...
		# Save S3 URL in the Sales Invoice
		self.custom_pdf_url = s3_url
		self.db_update()
		#self.custom_pdf_url=pdf_filename
		frappe.msgprint(f"Sales Invoice PDF uploaded to S3: {s3_url}")
		...
		qr.add_data(self.custom_pdf_url)
		qr.make(fit=True)
		...
		# Save the base64 encoded image to the custom field 'qr_code_image'
		qr_code_base64 = f"data:image/png;base64,{img_str}"
		
		# Log the QR code data (optional for troubleshooting)
		print("Generated QR Code Base64: ", qr_code_base64)
		
		# Store it in the document field
		self.custom_qr_code = qr_code_base64
